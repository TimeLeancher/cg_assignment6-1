...
void render_rasterized() {
    ...
    mat4 view_matrix = lookAt(vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, -1.0f), vec3(0.0f, 1.0f, 0.0f));
    ...
    for (size_t i = 0; i < SphereGenerator::gIndexBuffer.size(); i += 3) {
        ...
        for (int k = 0; k < 3; ++k) {
			...
			vec4 p_camera = view_matrix * p_world; //카메라 공간 좌표
            ...
            for (int x_px = min_x; x_px <= max_x; ++x_px) {
                ...
                if (bc.x >= 0.0f && bc.y >= 0.0f && bc.z >= 0.0f) {
                    ...
                    if (current_depth < ZBuffer[y_px * Width + x_px] && current_depth >= 0.0f && current_depth <= 1.0f) { // 깊이 검사 및 범위 확인 //현재픽셀이 이전픽셀보다 가까우면 갱신
                        ...
                        vec3 color_before_gamma = flat_color;
                        vec3 final_color;
						final_color.r = pow(std::max(0.0f,color_before_gamma.r), 1.0f / 2.2f); // 감마 보정
						...
						final_color = glm::clamp(final_color,0.0f, 1.0f); // 색상 클램핑
